...
\usepackage{titlesec}
%\titleformat{\part}{\bfseries\centering\sffamily}{\huge\Roman{part}.}{10pt}{\huge}
%\titleclass{\part}{top}
%\titlespacing{\part}{0pt}{0pt}{0.5in}
%\titleformat{\chapter}{\bfseries\sffamily}{\Large\arabic{chapter}.}{10pt}{\Large}
%\titleclass{\chapter}{straight}
%\titlespacing{\part}{0.5in}{0pt}{0.5in}
...
and used interchangably while they shouldn't. This section aims to clarify
...
represents, but is meerly a choice of storage.
...
for images since it optimizes memory contigency for nearby reads.
...
compose of rows while using column vectors. The mismatch only matters if
one wish to use SIMD, and using same row / column composition will result in
simple linear combinations while mismatch will mean memory contigency for dot
products.
...
behaves differently in spatial inversions, and is actually shorthand for
tensors with Levi-Civita Symbol.
\begin{equation}
\begin{split}
...
elementa that are the most complicated, with two terms mixing everything.
...
When at "Gimbal Lock" Type \Rom{1} element goes to 1, Type \Rom{2} elements go to 0. Type
\Rom{3} elements give the relation of $\theta_0\pm\theta_2$, depending on the order of axis.
Thus Type \Rom{3} elements stores all the information of the matrix when at "Gimbal Lock".
...
"Gimbal Lock" $\theta_1$ via $\arctantwo$, giving $\theta_1$ with $\mathtt{epsilon}$ order error.
But the behavior is still erratic by requiring special treatment for "Gimbal Lock".
...
"Gimbal Lock" is CERN CLHEP's method. But when tried out seemd to be less
...
The solutions do not need to work on "Gimbal Lock" either, since they work
