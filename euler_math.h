...
public:
	static constexpr tquaternion<T> from_real_pure(T real, const tvector3<T>& pure)
	{
		tquaternion<T> q;
		q.w = real;
		q.x = pure.x;
	...
	static constexpr tquaternion<T>	wxyz(T w, T x, T y, T z)
	{
		return 	tquaternion<T>{w, x, y, z};
	}

	static constexpr tquaternion<T>	xyzw(T x, T y, T z, T w)
	{
		return 	tquaternion<T>{w, x, y, z};
	}
	...
	constexpr tvector3<T> pure() const
	{
...
public:
	...
	constexpr T operator()(size_t row, size_t column) const
	{
		return m[row][column];
	}
...
template<typename T>
tmatrix3<T> operator*(const tmatrix3<T>& a, const tmatrix3<T>& b)
{
...
template<typename T>
tmatrix3<T> rotationX_mat3(T angle)
{
	tmatrix3<T> m;
	T c = cos(angle);
	T s = sin(angle);
...
template<typename T>
tmatrix3<T> rotationY_mat3(T angle)
{
	tmatrix3<T> m;
	T c = cos(angle);
	T s = sin(angle);
...
template<typename T>
tmatrix3<T> rotationZ_mat3(T angle)
{
	tmatrix3<T> m;
	T c = cos(angle);
	T s = sin(angle);
...
template<typename T>
constexpr tmatrix3<T> to_mat3(const tquaternion<T>& q)
{
	tmatrix3<T> m;
	T xx = q.x * q.x;
	T yy = q.y * q.y;
...
tmatrix3<T> to_mat3(const teuler_angles<type, T>& euler)
{
...
teuler_angles<type, T> to_euler_any(const tmatrix3<T>& m)
{
	constexpr int i = euler_angles::get_i_axis(type);
	constexpr int j = euler_angles::get_j_axis(type);
	constexpr int k = euler_angles::get_k_axis(type);
...
template<euler_angle_type type, typename T>
teuler_angles<type, T> to_euler(const tmatrix3<T>& m)
{
	return to_euler_any<type, T, false>(m);
}

template<euler_angle_type type, typename T>
teuler_angles<type, T> to_euler_other(const tmatrix3<T>& m)
{
	return to_euler_any<type, T, true>(m);
}

template<euler_angle_type type, typename T>
tquaternion<T> to_quaternion(const teuler_angles<type, T>& euler)
{
	constexpr int i = euler_angles::get_i_axis(type);
	constexpr int j = euler_angles::get_j_axis(type);
	constexpr int k = euler_angles::get_k_axis(type);
...
template<euler_angle_type type, typename T, bool use_other = false>
teuler_angles<type, T> to_euler_keep_sign_any(const tquaternion<T>& q)
{
	constexpr int i = euler_angles::get_i_axis(type);
	constexpr int j = euler_angles::get_j_axis(type);
	constexpr int k = euler_angles::get_k_axis(type);
...
template<euler_angle_type type, typename T>
teuler_angles<type, T> to_euler_keep_sign(const tquaternion<T>& q)
{
	return to_euler_keep_sign_any<type, T, false>(q);
}

template<euler_angle_type type, typename T>
teuler_angles<type, T> to_euler_keep_sign_other(const tquaternion<T>& q)
{
	return to_euler_keep_sign_any<type, T, true>(q);
}

template<euler_angle_type type, typename T>
teuler_angles<type, T> to_euler(const tquaternion<T>& q)
{
...
template<euler_angle_type type, typename T>
teuler_angles<type, T> to_euler_other(const tquaternion<T>& q)
{
